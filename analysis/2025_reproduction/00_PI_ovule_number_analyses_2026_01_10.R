...
table1 <- read.csv("Table1_2026_01_09.csv")
...
phy <- read.tree("All_PI_pairs_Zuntini_2025_10.tre") # the is a tree with brach lengths for all phylogenetically independent pairs of species in table 1 - see below for code to produce this
...
# add values for genera with no data 
table2_plus[table2_plus$genus == "Sinalliaria",]$ovule_number_plus <- 22 # Use best estimate from Table2b for genus Alliaria as it is the most closely related
table2_plus[table2_plus$genus == "Achimenes",]$ovule_number_plus <- 1409.428571 # Use estimate from family Gesneriaceae as numbers are fairly consistent (7 species, min=526, max=2975)
table2_plus[table2_plus$genus == "Diplacus",]$ovule_number_plus <- 549.872222 # Use best estimate from Table2b for genus Erythranthe as it is the most closely related
...
)] <- 21036.52664 # Use best estimate of Orchidaceae from Table2b
...
table1_plus2 <- table1_plus %>%  mutate(mean_ov = (ovule_number_plus.x+ovule_number_plus.y)/2,
         ...
         ov_cat = case_when(ov_cat.x=="few" & ov_cat.y=="few" ~ "few",
                            ov_cat.x=="many" & ov_cat.y=="many" ~ "many",
                                  .default = "intermediate"), # this is when one species is few and the other is many
         # added this category because original life history category was giving weird results where "intermediate" had lower RI than both other types
